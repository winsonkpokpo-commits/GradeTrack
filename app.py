...
Fichier corrigé et prêt à être utilisé avec Streamlit.
"""
...
def charger_donnees() -> List[Dict[str, Any]]:
    """Charge les données depuis le CSV et renvoie une liste de dictionnaires.
    Si le fichier est absent ou illisible, renvoie une liste vide.
    """
    if os.path.exists(CSV_PATH):
        try:
            df = pd.read_csv(CSV_PATH)
            # S'assurer que les colonnes essentielles existent (ajouter si manquantes)
            for col in EXPECTED_COLS:
                if col not in df.columns:
                    df[col] = None
            # Reindexer dans l'ordre attendu
...
def sauvegarder_donnees() -> None:
    """Sauvegarde le contenu de st.session_state.data dans le CSV."""
    try:
        df = pd.DataFrame(st.session_state.data)
        # S'assurer des colonnes et de l'ordre
        for col in EXPECTED_COLS:
            if col not in df.columns:
                df[col] = None
        df = df.reindex(columns=EXPECTED_COLS)
        # Écriture atomique simple : écrire dans un fichier temporaire puis remplacer
...
def ajouter_note(eleve: str, matiere: str, note: float, coef: float, trimestre: str) -> None:
    """Ajoute une note (sous forme de dict) dans st.session_state.data puis sauvegarde."""
    # Validation minimale
...
def supprimer_note(note_id: str) -> None:
    """Supprime une note par son id, sauvegarde et relance l'app."""
...
def calcul_moyenne(df: pd.DataFrame) -> Optional[float]:
    """Calcule la moyenne pondérée (Note * Coef) / somme(coef).
    Retourne None si impossible (pas de notes ou somme des coefficients = 0).
    """
...
if selected == "-- Nouvel élève --":
    ...
    if st.sidebar.button("➕ Créer l'élève"):
        if not new_name or new_name.strip() == "":
            ...
            # On l'ajoute comme une entrée sans notes pour qu'il apparaisse dans la liste
...
with st.form("form_ajout_note", clear_on_submit=True):
    ...
    # Si l'utilisateur a choisi de créer un nouvel élève et vient de saisir un nom, on le prend
...
with tab1:
    ...
    if not df_eleve.empty:
        # Affichage par ligne avec bouton supprimer
...
if not df_eleve.empty:
    export_df = df_eleve.drop(columns=['id']) if 'id' in df_eleve.columns else df_eleve
